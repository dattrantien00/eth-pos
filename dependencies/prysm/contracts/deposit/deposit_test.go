...
func TestVerifyDepositSignature_ValidSig(t *testing.T) {
	...
	fmt.Println(hex.EncodeToString(params.BeaconConfig().DomainDeposit[:]))
	fmt.Println(hex.EncodeToString(params.BeaconConfig().GenesisForkVersion[:]))
	fmt.Println(hex.EncodeToString(params.BeaconConfig().ZeroHash[:]))
	fmt.Println("================================")
	require.NoError(t, err)
