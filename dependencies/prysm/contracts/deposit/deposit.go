...
func VerifyDepositSignature(dd *ethpb.Deposit_Data, domain []byte) error {
	...
	logrus.Warn("==========================++++++++++++++++++")
	logrus.Println(params.BeaconConfig().MaxEffectiveBalance)
	logrus.Println(hex.EncodeToString(params.BeaconConfig().GenesisForkVersion[:]))
	// logrus.Println(hex.EncodeToString(params.BeaconConfig()))
	logrus.Println(params.BeaconConfig().Eth1FollowDistance, params.BeaconConfig().EpochsPerEth1VotingPeriod)
	logrus.Warn("=================+++++++++++++++++++++++++++")
	...
	fmt.Println(hex.EncodeToString(ctrRoot[:]))
