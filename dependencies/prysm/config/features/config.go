...
func configureTestnet(ctx *cli.Context) error {
	if ctx.Bool(PraterTestnet.Name) {
		...
		logrus.Infoln(params.MainnetConfig().EpochsPerEth1VotingPeriod, params.MainnetConfig().Eth1FollowDistance)
		config := params.MainnetConfig()
		config.EpochsPerEth1VotingPeriod = 1
		config.Eth1FollowDistance = 1
		if err := params.SetActive(config.Copy()); err != nil {
			return err
		}
	}
	return nil
