...
#     exit 1
# fi
bootnode_enode=enode://00b3e6ddf28453510058a4b0f5ad4dedfebc714d4c9d358746cbf878e528127beffaabbf8a33368e021726d4b39439bdcbf279893a21c68457c5b3f049e3b4a9@10.9.0.47:0?discport=30301
...
# --deposit-json-file=./deposit_account/account1/validator_keys/deposit_data.json
...
i=1
 # Check if the PRYSM_BOOTSTRAP_NODE variable is already set
    if [[ -z "${PRYSM_BOOTSTRAP_NODE}" ]]; then
        ...
        PRYSM_BOOTSTRAP_NODE=$(curl -s 10.9.0.47:4100/eth/v1/node/identity | jq -r '.data.enr')
    ...
    VALIDATOR_DIR=./validator_wallet/validator$i
    ...
    $GETH_BINARY \
      ...
      --authrpc.vhosts="*" \
      --authrpc.addr=10.9.0.47 \
      --authrpc.jwtsecret=$NODE_DIR/execution/jwtsecret \
      --authrpc.port=$((GETH_AUTH_RPC_PORT + i)) \
      --datadir=$NODE_DIR/execution \
      --allow-insecure-unlock --keystore=$NODE_DIR/execution/keystore \
      --password=$geth_pw_file \
      --bootnodes=$bootnode_enode \
      --identity=node-$i \
      --maxpendpeers=$NUM_NODES \
      --verbosity=3 \
      --syncmode=snap --gcmode=archive > "$NODE_DIR/logs/geth.log" 2>&1 &
    ...
    $PRYSM_BEACON_BINARY \
      ...
      --grpc-gateway-host=10.9.0.47 \
      --grpc-gateway-port=$((PRYSM_BEACON_GRPC_GATEWAY_PORT + i)) \
      --execution-endpoint=http://10.9.0.47:$((GETH_AUTH_RPC_PORT + i)) \
      --accept-terms-of-use \
    ...
    --beacon-rpc-provider=10.9.0.47:$((PRYSM_BEACON_RPC_PORT + i)) \
    ...
    --graffiti="node-$i" \
    --wallet-dir=$VALIDATOR_DIR \
    --proposer-settings-file=./validator0.json \
...
tail -f "$NETWORK_DIR/node-$i/logs/geth.log"
